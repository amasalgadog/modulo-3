...
export async function run() {
    ...
    if (executable.isNothing()) {
        executable = Maybe.of(await installDependencyCheck(cli.binDir, cli.odcVersion, cli.proxyUrl, cli.githubToken, cli.forceInstall));
    }
    else {
        log("Locally preinstalled (OWASP_BIN) Dependency-Check Core found.");
    }
    if (executable.isJust()) {
        const result = await executeDependencyCheck(executable.unsafeCoerce(), cli.cmdArguments, cli.outDir, cli.proxyUrl, cli.hideOwaspOutput);
