...
const command = program
    ...
    .option("--odc-version <version>", 'the version of the dependency-check CLI to install in format "v1.2.3"')
    .option("-p, --proxy <url>", "the URL to a proxy server in the format http(s)://[user]:[password]@<server>:[port]", parseProxyUrl)
    .addOption(new Option("--github-token <token>", "GitHub token to authenticate against API").env("GITHUB_TOKEN"))
    .addOption(new Option("--nvdApiKey <key>", "NVD API key to authenticate against API").env("NVD_API_KEY"))
    .addOption(new Option("--project <name>", "the name of the project being scanned").env("PROJECT_NAME"))
    .addOption(new Option("--owasp-bin <path>", "the path to a preinstalled dependency-check-cli binary")
    .env("OWASP_BIN")
    .argParser(parseOwaspBinary))
    .option("--hide-owasp-output", "do not display the output of the dependency-check-cli binary", false)
    .option("--ignore-errors", "always exit with code 0", false)
    .option("-d, --data <path>", "the location of the data directory used to store persistent data", path.join(os.tmpdir(), "dependency-check-data"))
...
function buildCmdArguments() {
    ...
    args.push("--project", getProjectName().orDefaultLazy(getProjectNameFromPackageJson));
...
function parseOwaspBinary(value) {
